...
from lumfunc_new import bootstrap_Cminus
...
x_dist, cumlx_dist, dx_dist, y_dist, cumly_dist, dy_dist = bootstrap_Cminus(x, y, xmax, ymax,
                                                    x_fit, y_fit,
                                                    Nbootstraps=20,
                                                    normalize=True, get_cumulative=True)
...
# Second subplot is the true & inferred 1D distributions
ax = fig.add_subplot(132)
ax.plot(x_mid, x_pdf.cdf(x_mid), '-k', label='$cdf(x)$')
ax.plot(y_mid, y_pdf.cdf(y_mid), '--k', label='$cdf(y)$')

ax.errorbar(x_mid, cumlx_dist/cumlx_dist[-1], dx_dist, fmt='ok', ecolor='k', lw=1, ms=4)
ax.errorbar(y_mid, cumly_dist/cumly_dist[-1], dy_dist, fmt='^k', ecolor='k', lw=1, ms=4)
ax.legend(loc='lower right')

ax.set_ylim(0, 1.2)
ax.set_xlim(0, 1)
ax.set_xlabel('$x$, $y$')
ax.set_ylabel('cumulatve distribution')

# Third subplot is the "observed" 2D distribution
